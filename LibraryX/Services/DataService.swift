...
class DataService{
    ...
    func getSavedEbooks(uid: String, handler: @escaping(_ handler: [EbookDetailCell])->()){
        ...
        REF_USER.child(uid).child("savedEbooks").queryOrdered(byChild: "time").observe(DataEventType.value, with: { (snapshot) in
                    ...
                    for ebookCompleteDetail in snapshot{
                        ...
                        if let year = ebookCompleteDetail.childSnapshot(forPath: "year").value as? String {
                            unwrappedYear = year
                        }
    ...
    func checkIfHaveSavedEbooks(uid: String, handler: @escaping (_ handler: String)->()){
        REF_USER.child(uid).child("savedEbooks").queryOrdered(byChild: "image").observe(DataEventType.value) { (snapshot) in
            if snapshot.exists(){
                handler("yes")
            }else{
                handler("no")
    ...
    func get10RecentActivities(uid: String, handler: @escaping(_ eachBookObj: [BookTitleForProfileCell])->()){
        REF_USER.child(uid).child("mybooks").queryOrdered(byChild: "actualReturned").queryLimited(toLast: 10).observe(DataEventType.value) { (snapshot) in
    ...
    func getABookStatus(imgTitleInMS: Double, handler: @escaping(_ status: [String])->()){
        REF_BOOK.queryOrdered(byChild: "image").queryEqual(toValue: imgTitleInMS).observe(DataEventType.value) { (snapshot) in
            ...
            for book in snapshot{
                ...
                print(status)
                self.REF_USER.child((Auth.auth().currentUser?.uid)!).child("mybooks").queryOrdered(byChild: "bookImgTitleInMS").observe(DataEventType.value, with: { (snapshot2) in
                    guard let snapshot2 = snapshot2.children.allObjects as? [DataSnapshot] else {return}
                    var latestBookStatus = ""
                    for myBook in snapshot2{
                        latestBookStatus = myBook.childSnapshot(forPath: "status").value as! String
                    }
                    ...
                    theBook.append(latestBookStatus)
    ...
    func borrowBook(imgTitleInMS: Double, uid: String, title: String, start: Double, until: Double){
        REF_BOOK.queryOrdered(byChild: "image").queryEqual(toValue: imgTitleInMS).observeSingleEvent(of: DataEventType.value, with: { (snapshot) in
            ...
            for book in snapshot{
                bookKey = book.key
                qty = book.childSnapshot(forPath: "qty").value as! Int
                qty -= 1
                self.REF_USER.queryOrdered(byChild: "uid").queryEqual(toValue: (Auth.auth().currentUser?.uid)!).observeSingleEvent(of: DataEventType.value, with: { (snapshot) in
                    ...
                    if qty == 0{
                        self.REF_BOOK.child(bookKey).updateChildValues(["start":start, "qty":qty, "until":until, "status":"no", "borrowedBy":(Auth.auth().currentUser?.uid)!])
                        
                    }else{
                        self.REF_BOOK.child(bookKey).updateChildValues(["start":start, "qty":qty, "until":until, "status":"avail", "borrowedBy":(Auth.auth().currentUser?.uid)!])
    ...
    func returnBook(imgTitleInMS: Double, uid: String, title: String, actualReturned: Double){
        ...
        REF_BOOK.queryOrdered(byChild: "image").queryEqual(toValue: imgTitleInMS).observeSingleEvent(of: DataEventType.value, with: { (snapshot) in
            ...
            for book in snapshot{
                ...
                qty = book.childSnapshot(forPath: "qty").value as! Int
                qty += 1
                self.REF_BOOK.child(bookKey).updateChildValues(["actualReturned":actualReturned,"qty":qty,"status":"avail"])
            }
            print(bookKey)
